...
import { ReferenceCountGarbageCollector } from './rcgc';
...
export type Value = boolean | BigInt | Pointer | number;
...
function evalCall(instr: bril.Operation, state: State): Action {
  ...
  if (!('dest' in instr)) {  // `instr` is an `EffectOperation`.
    ...
    // RCGC - 4. If the return value is a pointer, assign it for gc
    if (retVal.hasOwnProperty('loc')) {
      state.gc.assign(instr.dest, state.env, (retVal as Pointer).loc);
    }

    state.env.set(instr.dest, retVal);
  }
  state.gc.decEnv(newEnv);
  return NEXT;
}
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  case "id": {
    ...
    // RCGC - 5. On an id, dec the old dest count while incrementing the new reference count
    if (val.hasOwnProperty("loc")) {
      state.gc.assign(instr.dest, state.env, (val as Pointer).loc);
    }
  ...
  case "alloc": {
    ...
    // RCGC - 1. If there is a new allocation, add it to gc
    state.gc.new(ptr.loc);
  ...
  case "ptradd": {
    ...
    // RCGC - 3. On ptradd, dec the old dest count while incrementing the new reference count
