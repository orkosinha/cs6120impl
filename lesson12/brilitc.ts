...
function trace(instr: bril.Instruction, state: State): void {
  ...
  if (stop_tracing_ops.includes(instr.op)) {
    ...
    // Force exit speculation
    let bool_name = "false_guard_const";
    state.trace.push({
      op: "const",
      type: "bool",
      value: false,
    ...
    if (getBool(instr, state.env, 0)) {
      ...
      let neg_bool = "negative_" + instr.args![0];
      state.trace.push({
        op: "not",
        type: "bool",
        args: [instr.args![0]],
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  case "print": {
    ...
    // Skip printing for integrating with brench
    // console.log(...values);
    return NEXT;
  }
...
function evalFunc(func: bril.Function, state: State): Value | null {
  for (let i = 0; i < func.instrs.length; ++i) {
    ...
    if ('op' in line) {
      ...
      // Encountered the first label
      if (state.tracing) {
        if (state.traced_line.includes(line)) {
          state.trace.push({
            op: "commit"
          });
          state.trace.push({
...
type Trace = { trace: bril.Instruction[], program: bril.Program}
